...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT u FROM User u WHERE u.isLineManager = TRUE AND u.status = 'ACTIVE'")
    public List<User> getUserLineManager();

    @Query("SELECT u FROM User u WHERE u.isTeamLead = TRUE AND u.status = 'ACTIVE'")
    public List<User> getUserTeamLead();

    @Query("SELECT u FROM User u WHERE u.isMentor = TRUE AND u.status = 'ACTIVE'")
    ...
    @Query("SELECT u FROM User u INNER JOIN UserTeam ut ON u.userId = ut.userId WHERE u.userId !=?1    AND ut.teamId IN (SELECT ut2.teamId FROM UserTeam ut2 WHERE ut2.userId = ?1) AND NOT EXISTS (SELECT 1 FROM Appraisal a WHERE a.type = 'TEAMMATE' AND ( a.evaluatedPerson.userId = u.userId  OR (a.appraiser.userId = u.userId AND a.evaluatedPerson.userId = ?1)))")
    public List<User> findTeamMatesNoAppariserBy(Integer userId);

    @Query("SELECT u FROM User u INNER JOIN UserGroup ug ON u.userId = ug.userId WHERE u.userId !=?1    AND ug.groupId IN (SELECT ug2.groupId FROM UserGroup ug2 WHERE ug2.userId = ?1) AND NOT EXISTS (SELECT 1 FROM Appraisal a WHERE a.type = 'GROUPMATE' AND ( a.evaluatedPerson.userId = u.userId  OR (a.appraiser.userId = u.userId AND a.evaluatedPerson.userId = ?1)))")
