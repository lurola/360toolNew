...
function assignMentor() {
	...
	$.get("/user/assignMentor", {userId : $('#userId').val(), mentorUserId : $('#mentorUserId').val()}, function(data, status, xhr){
		window.location = "/";
	});
...
<div id="mentorDiv" class="container" style="display:none">
	<h3>Mentor</h3>

	<select name="mentorOfficeId" onchange="officeSelected(this);"  class="form-control" id="mentorOfficeId" required>
		<#list model["officeList"] as office2>
			<option  value="${office2.officeId}">${office2.officeName}</option>
		</#list>
	</select>	
	
	<select name="mentorUserId"  class="form-control" id="mentorUserId" required></select>	
