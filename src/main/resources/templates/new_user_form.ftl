...
function showMentorCombo() {
	...
	$.get("/user/getUserByOfficeId", {officeId : $('#mentorOfficeId').val()}, function(data, status, xhr){
		...
		if(data.length === 'undefined') {
	    	...
	    	var $select = $('#mentorId'); //  empty it first with .empty()
...
<div class="container">
	...
	<form  name="form" action="" method="post">
		...
		<div class="col-md-10">
			<select name="groupId" id="groupId" required multiple  class="form-control">
				...
				<#list model.userForm.groupId as listaGruposElegidos> <#if
					group.groupId== listaGruposElegidos>
		...
		<div id="mentorDiv" name="mentorDiv" class="row" >
			<div class="col-md-2">
				<label for="mentorId">Mentor User</label> 
			</div>
			<div class="col-md-10">
	...
	<#if model["signUp"] == false>
		...
		<div class="col-md-10">	
			<select name="appRole"
				id="appRole" required  class="form-control">
		...
		<#else>
			<button class="btn btn-primary" formaction="/user/signup" formmethod="post">Save</button>
		</#if>
